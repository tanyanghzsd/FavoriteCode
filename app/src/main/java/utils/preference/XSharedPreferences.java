...
public class XSharedPreferences extends ContentProvider implements android.content.SharedPreferences {
                ...
                if (sAuthorityUrl == null) {
                    ...
                    if (packageInfos != null && packageInfos.providers != null) {
                        for (ProviderInfo providerInfo : packageInfos.providers) {
                            if (providerInfo.name.equals(XSharedPreferences.class.getName())) {
                    ...
                    if (sAuthoriry == null) {
                        throw new IllegalArgumentException("'AUTHORITY' initialize failed, Unable to find explicit provider class " + XSharedPreferences.class.getName() + "; have you declared this provider in your AndroidManifest.xml?");
                    }
     ...
     */
    public static XSharedPreferences getSharedPreferences(Context context, String name, int mode) {
        return new XSharedPreferences(context, name, mode);
    }
    ...
    private XSharedPreferences(Context context, String name, int mode) {
        mContext = context;
        mName = name;
        mMode = mode;
    ...
    public void registerOnSharedPreferenceChangeListener(OnSharedPreferenceChangeListener listener) {
        synchronized (this) {
            if (mListeners == null) {
                mListeners = new HashMap<OnSharedPreferenceChangeListener, Object>();
            }
                            ...
                            if (mName.equals(name) && keysModified != null) {
                                ...
                                for (int i = keysModified.size() - 1; i >= 0; i--) {
                                    ...
                                    for (OnSharedPreferenceChangeListener listener : listeners) {
                                        if (listener != null) {
                                            listener.onSharedPreferenceChanged(XSharedPreferences.this, key);
                                        }
                                    }
                                }
                            }
    ...
    private String makeAction(String name) {
        return String.format("%1$s_%2$s", XSharedPreferences.class.getName(), name);
    }
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        String name = uri.getPathSegments().get(0);
        int mode = Integer.parseInt(selectionArgs[0]);
        android.content.SharedPreferences preferences = getContext().getSharedPreferences(name, mode);
...
//                            ReflectionUtil.editorApply(editor); // Android 2.3
                            ...
                            // XSharedPreferences instance back, which has the
